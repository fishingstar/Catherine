...
namespace Catherine
{
	...
	void Mesh::SetupBuffers(const std::vector<Vertex> & vertex, const std::vector<unsigned int> & index)
	{
		...
		tmp_attributes.emplace_back(2, 4, ValueType::Float, false, sizeof(Vertex), 6 * sizeof(float));
		tmp_attributes.emplace_back(3, 2, ValueType::Float, false, sizeof(Vertex), 10 * sizeof(float));
